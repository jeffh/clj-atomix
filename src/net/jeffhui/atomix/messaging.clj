(ns net.jeffhui.atomix.messaging
  (:require [net.jeffhui.atomix.utils :as utils :refer [->member-id *default-serializer*]])
...
(defn send!
  ...
  (let [s *default-serializer*]
    (if (sequential? target)
      (.multicast (.getCommunicationService agent) (str subject) message
                  (utils/func #(.encode s %))
                  ^java.util.Set (set (mapv ->member-id target)))
